...
"""
eccentricity dependent RFs moved to a spearate LGN module, with V1 as a normal block
backward connections as a ConvTranspose
skip connection recombined after lateral and backward instead of before.
"""
...
class CogBlock(nn.Module):
    ...
    def __init__(self, channels, kernel_size=3, stride=1, scale=4):
        ...
        if self.prev_channels != self.in_channels:
            self.conv_back = nn.ConvTranspose2d(oc, pc, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, inputs):
        ...
        f = self.fl(torch.concat([f, l], dim=1)) if l is not 0 else f # TODO: do this in one operation in future
        f = self.fb(torch.concat([f, b], dim=1)) if b is not 0 else f
...
class CogNet(nn.Module):
    ...
    def forward(self, inp):
        ...
        empty_state = {'f': 0, 'l': 0, 'b': 0, 'i': 0} # change these to empty tensors as part of above TODO
        ...
        for c in range(cycles):
            ...
            for blk in blocks:
                ...
                i = states[prv_blk]['i'] if blk != 'LGN' else 0
