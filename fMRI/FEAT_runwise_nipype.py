...
def FEAT_runwise(num_procs=None):
    ...
    for scan in [op.basename(x) for x in glob.glob(f"{os.getcwd()}/derivatives/FEAT/designs/runwise/*")]:
        ...
        run_paths = sorted(glob.glob(f"{os.getcwd()}/derivatives/fmriprep-*/**/*/*{scan}*_desc-preproc_bold.nii.gz", recursive=True))
        ...
        for run_path in run_paths:
            ...
            if run_feat:
                ...
                design = design.replace('/mnt/HDD2_16TB', op.expanduser(f'~/david'))
    ...
    print(f'{num_procs} processes simultaneously')
    for batch in range(math.ceil(len(design_paths) / num_procs)):
        processes = []
        first_job = batch * num_procs
        last_job = min(((batch + 1) * num_procs), len(design_paths) - 1)
    ...
    for design_path in design_paths:
        print(f'{datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")} | {job}')
        os.system(f'feat {design_path}')
    '''

if __name__ == "__main__":

    os.chdir(op.expanduser("~/david/projects/p022_occlusion/in_vivo/fMRI/v4"))
    num_procs = 10
